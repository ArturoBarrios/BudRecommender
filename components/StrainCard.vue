...
<template>
  <div class="border rounded-xl p-4 shadow-sm hover:shadow-md transition relative">
    ...
    <div v-if="showPreferencePopup"
      ...
      <button @click="updatePreference('like')" type="button" class="text-sm font-bold px-3 py-1 rounded transition"
        :class="isFavorite ? 'bg-green-600 text-white' : 'bg-green-100 text-green-700 hover:bg-green-200'">
        💚
      </button>
      <button @click="updatePreference('dislike')" type="button" class="text-sm font-bold px-3 py-1 rounded transition"
    ...
    <div class="flex">
      <!-- Image -->
      <div v-if="strain.imageUrl" class="">
        <img v-if="strain.imageUrl" :src="strain.imageUrl" alt="Strain Image"
          class="w-full rounded-lg object-contain mb-2 max-h-48" />
        ...
        <div>
          ...
          <div class="mt-2 text-xs text-gray-600">
            ...
            <template v-if="!isEditing">
              ...
              <button class="mt-1 text-blue-600 underline text-xs hover:text-blue-800" @click="isEditing = true">
                ✏️ Edit Terpenes
              </button>
            </template>
            ...
            <template v-else>
              <div v-for="terpene in editableTerpenes" :key="terpene.id" class="flex items-center space-x-2 mt-1">
                ...
                <input type="number" v-model="terpene.percentage" class="border px-1 py-0.5 text-xs w-20 rounded" min="0"
                  max="100" step="0.1" />
